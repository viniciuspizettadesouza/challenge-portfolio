...
describe("challenge registry", () => {
  it("contains fourteen curated entries backed by all twenty-three sources", () => {
    expect(challenges).toHaveLength(14);
    expect(new Set(challenges.map(({ slug }) => slug)).size).toBe(14);
  ...
  it("resolves every unique legacy slug to its canonical entry", () => {
    expect(challengeAliases.size).toBe(36);

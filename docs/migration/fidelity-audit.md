...
| User Management | React 18, React Router, ReqRes | React island with deterministic local service | Historical Part 1 remains preserved; the maintained demo uses a behavioural equivalent rather than literal ReqRes HTTP, fixes independent sign-in and token enforcement, completes CRUD with six-user pagination, keeps mutations in memory, persists theme preference across application reopening, adds explicit Parts 1–4 tests and Docker, and lands on the dashboard with the retained greeting |
